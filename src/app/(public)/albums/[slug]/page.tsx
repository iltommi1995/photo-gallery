...
export default async function AlbumPage({ params }: AlbumPageProps) {
  ...
  // WEB only — this is canonical/SEO content describing the desktop
  // version of the album, independent of whatever a mobile visitor sees.
  const allPhotos = webChapters.flatMap((c) =>
    c.placements.flatMap((p) => (p.type === "PHOTO" ? [p.photo] : [])),
  );
  const jsonLd = {
  ...
  return (
    <>
      ...
      <AlbumScrollView
        ...
        mobileLandscapeChapters={mobileLandscapeChapters}
        mobilePortraitChapters={mobilePortraitChapters}
        showChapterLabels={album.showChapterLabels}
        chapterLayout={album.chapterLayout}
      />
    </>
  );
}
