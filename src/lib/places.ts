...
type LocatedPhoto = Pick<Photo, "id" | "locationName" | "takenAt" | "createdAt">;

export function groupPhotosByPlace<T extends LocatedPhoto>(photos: T[]) {
  const places = new Map<string, { slug: string; name: string; photos: T[] }>();
  for (const photo of photos) {
    ...
    const place = places.get(slug) ?? { slug, name, photos: [] };
